...
pub fn deriveCopyForEnum(enumDef: &Enum) -> Instance {
    let traitName = Identifier::new("Std.Ops.Basic.Copy".to_string(), enumDef.name.location());
    let instanceName = Identifier::new(format!("Copy_{}", enumDef.name.name()), enumDef.name.location());
