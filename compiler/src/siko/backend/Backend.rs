...
pub fn process(ctx: &ReportContext, runner: &mut Runner, program: Program) -> Program {
    ...
    let program = stage!(runner.child("dead_code_elimination"), {
        eliminateDeadCode(&ctx, program)
    });
    //println!("after dce\n{}", program);
    let program = stage!(runner.child("recursive_data_handling"), {
        RecursiveDataHandler::process(ctx, program)
    });
    //println!("after recursive data handling\n{}", program);
    let program = stage!(runner.child("field_ref_merging"), {
        FieldRefMerger::mergeFieldRefs(program)
    });
    //println!("after field ref merge\n{}", program);
    let dropCheckRunner = runner.child("drop_check");
    let program = stage!(dropCheckRunner.clone(), { checkDrops(&ctx, program, dropCheckRunner) });
    //println!("after dropcheck\n{}", program);
    // program
    //     .dumpToFile("hirdump/afterdropcheck")
    //     .expect("Failed to dump HIR");
    let cfg = runner.getConfig();
    let program = stage!(runner.child("monomorphizing"), {
    ...
    let program = stage!(runner.child("simplification"), {
    ...
    let program = stage!(runner.child("coroutine_lowering"), {
    ...
    let program = stage!(runner.child("tuple_removal"), { removeTuples(&program) });
    Check::new(&program).process(ctx);
    let program = stage!(runner.child("simplification2"), {
